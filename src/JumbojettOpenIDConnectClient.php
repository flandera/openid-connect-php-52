...
if (!function_exists('curl_init')) {
	throw new JumbojettOpenIDConnectClientException('OpenIDConnect needs the CURL PHP extension.');
}
if (!function_exists('json_decode')) {
	throw new JumbojettOpenIDConnectClientException('OpenIDConnect needs the JSON PHP extension.');
}
 ...
 *
 */
class JumbojettOpenIDConnectClient
{
	...
	/**
	 * @return bool
	 * @throws JumbojettOpenIDConnectClientException
	 */
	public function authenticate() {
		...
		if (isset($_REQUEST['error'])) {
			...
			throw new JumbojettOpenIDConnectClientException("Error: " . $_REQUEST['error'] .$desc);
		}
...
//			print_r($code);
			...
			if (isset($token_json->error)) {
				if (isset($token_json->error_description)) {
					throw new JumbojettOpenIDConnectClientException($token_json->error_description);
				}
				throw new JumbojettOpenIDConnectClientException('Got response: ' . $token_json->error);
			}

			// Do an OpenID Connect session check
			if ($_REQUEST['state'] != $this->getState()) {
				throw new JumbojettOpenIDConnectClientException("Unable to determine state");
			}
			...
			if (!property_exists($token_json, 'id_token')) {
				throw new JumbojettOpenIDConnectClientException("User did not authorize openid scope.");
			}
			...
			if ($this->canVerifySignatures()) {
				if (!$this->getProviderConfigValue('jwks_uri')) {
					throw new JumbojettOpenIDConnectClientException ("Unable to verify signature due to no jwks_uri being defined");
				}
				if (!$this->verifyJWTsignature($token_json->id_token)) {
					throw new JumbojettOpenIDConnectClientException ("Unable to verify signature");
				}
			} else {
			...
			if ($this->verifyJWTclaims($claims, $token_json->access_token)) {
				...
				throw new JumbojettOpenIDConnectClientException ("Unable to verify JWT claims");
			}
	...
	private function getProviderConfigValue($param, $default = null) {
		...
		if (!isset($this->providerConfig[$param])) {
			...
			if ($value) {
				...
				throw new JumbojettOpenIDConnectClientException("The provider {$param} has not been set. Make sure your provider has a well known configuration available.");
			}
...
//		print_r($auth_endpoint);
	...
	private function get_key_for_header($keys, $header) {
		...
		if (isset($header->kid)) {
			throw new JumbojettOpenIDConnectClientException('Unable to find a key for (algorithm, kid):' . $header->alg . ', ' . $header->kid . ')');
		} else {
			throw new JumbojettOpenIDConnectClientException('Unable to find a key for RSA');
		}
	}
...
//        if (!class_exists('\phpseclib\Crypt\RSA') && !class_exists('Crypt_RSA')) {
		if (!class_exists('Crypt_RSA')) {
			throw new JumbojettOpenIDConnectClientException('Crypt_RSA support unavailable.');
		}
		if (!(property_exists($key, 'n') and property_exists($key, 'e'))) {
			throw new JumbojettOpenIDConnectClientException('Malformed key object');
		}
...
//        	$rsa->loadKey($public_key_xml, Crypt_RSA::CRYPT_RSA_PUBLIC_FORMAT_XML);
			$rsa->loadKey($public_key_xml, CRYPT_RSA_PUBLIC_FORMAT_XML);
//        	$rsa->signatureMode = Crypt_RSA::CRYPT_RSA_SIGNATURE_PKCS1;
...
//        	$rsa->signatureMode = Crypt_RSA::CRYPT_RSA_SIGNATURE_PKCS1;
	...
	private function verifyHMACJWTsignature($hashtype, $key, $payload, $signature)
	{
		if (!function_exists('hash_hmac')) {
			throw new JumbojettOpenIDConnectClientException('hash_hmac support unavailable.');
		}
	...
	private function verifyJWTsignature($jwt) {
		...
		if ($jwks === NULL) {
			throw new JumbojettOpenIDConnectClientException('Error decoding JSON from jwks_uri');
		}
		$verified = false;
		switch ($header->alg) {
			case 'RS256':
			...
			case 'HS384':
				...
				break;
			default:
				throw new JumbojettOpenIDConnectClientException('No support for signature type: ' . $header->alg);
		}
		return $verified;
	}
...
//        print_r($claims->iss == $this->getProviderURL());
//		print_r(($claims->aud == $this->clientID) || (in_array($this->clientID, $claims->aud)));
//		print_r($claims->nonce == $this->getNonce());
	...
	protected function fetchURL($url, $post_body = null,$headers = array()) {
		...
		if ($output === false) {
			throw new JumbojettOpenIDConnectClientException('Curl error: ' . curl_error($ch));
		}
	...
	public function getProviderURL() {
		...
		if (!isset($this->providerConfig['issuer'])) {
			throw new JumbojettOpenIDConnectClientException("The provider URL has not been set");
		} else {
	...
	public function register() {
		...
		if ($json_response === false) {
			throw new JumbojettOpenIDConnectClientException("Error registering: JSON response received from the server was invalid.");
		} elseif (isset($json_response->{'error_description'})) {
			throw new JumbojettOpenIDConnectClientException($json_response->{'error_description'});
		}
		...
		if (isset($json_response->{'client_secret'})) {
			...
			throw new JumbojettOpenIDConnectClientException("Error registering:
...
//    protected function getNonce() {
	...
	{
		$len1=self::safeLength($str1);
		$len2=self::safeLength($str2);
